...
mycols2 = c("turquoise4",  "turquoise2",  "springgreen2", "yellowgreen", 
             ...
             "violet", "mediumpurple1", "blueviolet", "slateblue", "navyblue", "navy", "brown", "red", "pink", "orange")
...
# CDC Surveillance
 ...
 cv19_bymonth %>%
    ...
    filter(!is.na(vote_share) & date > ymd("2021-01-01") & date < ymd("2022-08-01")) %>%
  ...
  ggsave( here(paste("output/", 'FIG-1A-', Sys.Date(), '.png', sep="")),
          device = png(),
          plot = p1,
          width =17,
          bg = "white",
...
# CDC
 cv19_bymonth %>%
  ...
  ggsave( here(paste("output/", 'FIG-1B-', Sys.Date(), '.png', sep="")),
          device = png(),
          plot = p2,
          width =17,
          bg = "white",
...
# NIS-ACM  
  statev %>%
    ...
    filter(state_code != "DC") %>%
    filter(group_name == "All Adults 18+")  %>%
    ...
    ylim(c(0, 100)) + xlab("2020 vote share for the Democrats") + 
    xlim(c(0, 100)) + ylab("COVID-19 vaccination coverage") +
    scale_color_manual(values = mycols2[5:length(mycols2)]) +
    theme_pubclean() +
    stat_cor(method = "pearson", aes(label = ..r.label..),  
             label.x = 0,  show.legend = FALSE, label.y.npc = "bottom",   size=4) + 
...
cv19_bymonth %>%
  ...
  ggsave( here(paste("output/", 'FIG-S1-', Sys.Date(), '.png', sep="")),
          device = png(),
          plot = p5,
          width =17,
          bg = "white",
